...
def train_epoch(args, TrainImgLoader, model, model_fn, optimizer, epoch, iters, lr_scheduler):
    ...
    for batch_idx, data in enumerate(tbar):
        loss, loss_temporal, loss_reg, cur_psnr = model_fn(args, data, model, iters, epoch)
        optimizer.zero_grad()
        loss.backward()
        ...
        desc = 'Training  : Epoch %d, Avg. Loss = %.5f, learn ratio = %.7f, cur_psnr = %.5f' % (
            epoch, avg_train_loss, lr, cur_psnr)
        tbar.set_description(desc)
        tbar.update()
...
def load_pretrain(model, load_epoch):
    load_dir = args.NETS_DIR + '/checkpoint' + '_' + '%06d' % load_epoch + '.tar'
    print('Loading pre-trained checkpoint %s' % load_dir)
    model_state_dict = torch.load(load_dir)['state_dict']
    model.load_state_dict(model_state_dict)
...
if __name__ == '__main__':
    ...
    ten1 = torch.rand(1, 3, 256, 256).cuda()
    ten2 = torch.rand(1, 6, 256, 256).cuda()
    flops = FlopCountAnalysis(model, (ten1, ten2))
    print("####################FLOPs: ", flops.total())
    start = time.time()
    result = model(ten1, ten2)
    end = time.time()
    ...
    load_pretrain(model, args.LOAD_EPOCH)
    ...
    loss_fn = VGGPerceptualLoss().cuda()
    ...
    lr_scheduler = CosineAnnealingLR_warmup(args, optimizer, base_lr=learning_rate, last_epoch=iters - 1, min_lr=1e-7)
