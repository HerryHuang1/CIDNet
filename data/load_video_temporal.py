...
class data_loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            if self.loader == 'crop':
                # res_x = random.randint(self.args.WIDTH // 2, self.args.WIDTH * 2)
                # res_y = random.randint(self.args.HEIGHT // 2, self.args.HEIGHT * 2)
                # res_p = random.uniform(0.9, 1.1)
                res_p = 1
                rot = random.randint(0, 4) * 90
                res_x = math.ceil(self.args.WIDTH * res_p)
                res_y = math.ceil(self.args.HEIGHT * res_p)
                x = random.randint(0, res_x - self.args.CROP_SIZE)
                y = random.randint(0, res_y - self.args.CROP_SIZE)
                flip_ph = random.randint(0, 1)
                flip_pv = random.randint(0, 1)
                if flip_ph > 0.5:
                    flip_ph = 1
                else:
                    flip_ph = 0
                if flip_pv > 0.5:
                    ...
                    flip_pv = 0
                labels = crop_loader(self.args.CROP_SIZE, self.args.CROP_SIZE, x, y, res_x, res_y, flip_ph, flip_pv,
                                     rot, path_tars)
                moire_imgs = crop_loader(self.args.CROP_SIZE, self.args.CROP_SIZE, x, y, res_x, res_y, flip_ph, flip_pv,
                                         rot, path_srcs)
                moire_imgs_aux = crop_loader(self.args.CROP_SIZE, self.args.CROP_SIZE, x, y, res_x, res_y, flip_ph,
                                             flip_pv, rot, path_src_auxs)
...
def crop_loader(crop_size_x, crop_size_y, x, y, res_x, res_y, flip_ph, flip_pv, rot, path_set, pad_size=100, pad=False):
    imgs = []
    for path in path_set:
        img = Image.open(path).convert('RGB')
        t_list = [transforms.RandomRotation(degrees=(rot, rot))]
